		...
		describe("Integration tests", function(){
			...
			describe("Connection management tests", function(){
				...
				it("Should attempt to authenticate and reconnect if the socket closes", function(done){
					...
					assert.lengthOf(ms._requests, 0, "Server has no outstanding requests");
					...
					setTimeout(function(){
						...
						assert.lengthOf(ms._requests, 1, "Server got one http request");
						var httpReq = ms._requests.shift();
						assert.ok(httpReq, "HTTP request is ok");
						assert.include(httpReq.url, socket._apiRoute + socket._tokenPath, "HTTP route contains socket token route");
